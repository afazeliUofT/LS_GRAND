...
def simulate_qpsk_residual_phase(
    bits: np.ndarray,
    state_path: np.ndarray,
    snr_db: float,
    rng: np.random.Generator,
    *,
    innovation_std_deg: float = 0.0,
    initial_std_deg: float = 0.0,
    true_label: str = "",
) -> ChannelSample:
    """QPSK/AWGN with a small residual Wiener phase term plus C4 state jumps.

    The decoder used in the gate is deliberately mismatched: it models only the
    sparse C4 jump.  This tests whether a modest residual tracking error destroys
    latent concentration without pretending to be a complete oscillator model.
    """
    x = bits_to_qpsk(bits)
    path = np.asarray(state_path, dtype=np.int8).reshape(-1)
    if x.size != path.size:
        raise ValueError("state path length mismatch")
    init = np.deg2rad(float(initial_std_deg)) * float(rng.standard_normal())
    step_std = np.deg2rad(float(innovation_std_deg))
    drift = init + np.cumsum(step_std * rng.standard_normal(x.size))
    mean = apply_state(x, path) * np.exp(1j * drift)
    n0 = float(10.0 ** (-float(snr_db) / 10.0))
    sigma = float(np.sqrt(n0 / 2.0))
    noise = sigma * (rng.standard_normal(x.size) + 1j * rng.standard_normal(x.size))
    y = mean + noise
