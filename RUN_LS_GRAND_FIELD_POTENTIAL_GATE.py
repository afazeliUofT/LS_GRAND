#!/usr/bin/env python3
"""Single WSL wrapper for the LS-GRAND field-potential gate.

Drop this file into /home/afazeli2006/LS_GRAND and run it from the VS Code
WSL terminal.  The script never executes a shell/terminal close command.
"""
...
PACKAGE_FILENAME = "LS_GRAND_Field_Potential_Gate_v1_0.zip"
PACKAGE_DIRNAME = "LS_GRAND_Field_Potential_Gate_v1_0"
EXPECTED_SHA256 = "07ef756cd4744b165a8c63508cc429ed41f4bf12441043268213825fb27ce0fe"
...
def discover_repo(runner: Runner) -> Path:
    ...
    try:
        ...
        raise RuntimeError(
            "Run this wrapper from the initialized WSL git repository "
            f"{EXPECTED_REPO_PATH}. Current directory: {cwd}"
        ) from exc
    if root != EXPECTED_REPO_PATH.resolve():
        raise RuntimeError(
            f"Refusing to operate in the wrong repository. Expected {EXPECTED_REPO_PATH}, found {root}."
        )
    remote = runner.capture(["git", "remote", "get-url", "origin"], cwd=root)
    if normalize_remote(remote).lower() != EXPECTED_REPOSITORY.lower():
        raise RuntimeError(
            f"Unexpected origin remote: {remote!r}. Expected repository {EXPECTED_REPOSITORY}."
        )
    return root
...
def candidate_download_dirs() -> list[Path]:
    out: list[Path] = []
    override = os.environ.get("LSGRAND_DOWNLOADS")
    if override:
...
def locate_package(runner: Runner) -> Path:
    found: list[Path] = []
    for directory in candidate_download_dirs():
        path = directory / PACKAGE_FILENAME
        if path.is_file():
            found.append(path)
    if not found:
        searched = "\n  - ".join(str(p / PACKAGE_FILENAME) for p in candidate_download_dirs())
        raise FileNotFoundError(
            f"Could not find {PACKAGE_FILENAME}. Searched:\n  - {searched}\n"
            "Download the ZIP without renaming it, or set LSGRAND_DOWNLOADS."
        )
    valid: list[Path] = []
    ...
    with zipfile.ZipFile(zip_path) as zf:
        for info in zf.infolist():
            ...
            if stat.S_ISLNK(mode):
                raise RuntimeError(f"symbolic links are not permitted in the package: {name}")
...
def verify_installed_tree(package_dir: Path) -> None:
    ...
    if failures:
        raise RuntimeError(
            "Existing extracted package differs from the frozen ZIP. Protect or revert local edits first. "
            f"Mismatches: {failures[:10]}"
        )
...
def install_package_tree(runner: Runner, repo: Path, zip_path: Path) -> Path:
    ...
    if package_dir.exists():
        ...
        runner.note(f"Moving an older package tree to {backup}")
    ...
    if not extracted.is_dir():
        raise RuntimeError("ZIP does not contain the expected single top-level package directory")
...
def ensure_root_gitignore(repo: Path) -> None:
    path = repo / ".gitignore"
    block = """
# LS-GRAND local execution environment and non-review artifacts
.venv/
__pycache__/
*.py[cod]
*.egg-info/
...
dist/
results/**/raw/
results/**/frame_traces/
results/**/checkpoints/
experiments/LS_GRAND_Field_Potential_Gate_v1_0/reference/proposal_extracted_for_audit.txt
""".lstrip()
    current = path.read_text() if path.exists() else ""
    marker = "# LS-GRAND local execution environment"
...
def check_clean_staging_area(runner: Runner, repo: Path) -> None:
    ...
    if staged.strip():
        raise RuntimeError(
            "The repository already has staged files. To avoid mixing unrelated work into the automated "
            "commit, commit or unstage them first. No files were changed by this wrapper yet.\n"
            f"Pre-existing staged paths:\n{staged}"
        )


def validate_profile() -> str:
    profile = os.environ.get("LSGRAND_PROFILE", "gate").strip().lower()
    if profile not in VALID_PROFILES:
        raise ValueError(f"LSGRAND_PROFILE must be one of {sorted(VALID_PROFILES)}, got {profile!r}")
    return profile


def stage_review_artifacts(runner: Runner, repo: Path, package_dir: Path, run_dir: Path, wrapper_path: Path) -> None:
    rel_package = package_dir.relative_to(repo)
    package_allowlist = [
        rel_package / "README.md",
        rel_package / "PACKAGE_METADATA.json",
        rel_package / "pyproject.toml",
        rel_package / "requirements.txt",
        rel_package / "SHA256SUMS",
        rel_package / "docs",
        rel_package / "theory",
        rel_package / "configs",
        rel_package / "src",
        rel_package / "tests",
        rel_package / "scripts",
        rel_package / "preflight_results" / "PRELIMINARY_EVIDENCE_SUMMARY.json",
        rel_package / "preflight_results" / "PREFLIGHT_FINAL_GATE_REPORT.md",
        rel_package / "preflight_results" / "PREFLIGHT_FINAL_GATE_VERDICT.json",
    ]
    paths: list[Path] = [repo / ".gitignore", wrapper_path]
    ...
    staged = runner.capture(["git", "diff", "--cached", "--name-only"], cwd=repo)
    bad = [line for line in staged.splitlines() if line == ".venv" or line.startswith(".venv/") or "/__pycache__/" in f"/{line}/"]
    if bad:
        runner.run(["git", "restore", "--staged", "--", *bad], cwd=repo, check=False)
        raise RuntimeError(f"Safety invariant violated: local environment files reached staging: {bad}")
...
def commit_and_push(runner: Runner, repo: Path, run_dir: Path) -> str | None:
    ...
    message = f"LS-GRAND field-potential gate: {run_dir.name}"
    runner.run(["git", "commit", "-m", message], cwd=repo)
    commit = runner.capture(["git", "rev-parse", "HEAD"], cwd=repo)
    if os.environ.get("LSGRAND_SKIP_PUSH", "0") == "1":
        runner.note(f"LSGRAND_SKIP_PUSH=1; commit {commit} was created locally but not pushed.")
        return commit
...
def main() -> int:
    stamp = utc_stamp()
    preliminary_log = EXPECTED_REPO_PATH / "results" / "_wrapper_logs" / f"wrapper_{stamp}.log"
    runner = Runner(preliminary_log)
    failure_path = preliminary_log.with_name(f"WRAPPER_FAILURE_{stamp}.json")
    try:
        runner.note("=" * 78)
        runner.note("LS-GRAND Field-Potential Gate v1.0")
        runner.note("=" * 78)
        ...
        unit_log = repo / "results" / "_wrapper_logs" / f"unit_tests_{stamp}.log"
        runner.run([python, "-m", "lsgrand.cli", "test", "--log", unit_log], cwd=package_dir, env=env)

        run_name = f"LS_GRAND_Field_Potential_Gate_{profile}_{stamp}"
        output_root = repo / "results"
        config = package_dir / "configs" / f"{profile}.json"
        runner.run(
            [
                python, "-m", "lsgrand.cli", "run",
                "--config", config,
        ...
        runner.run([python, package_dir / "scripts" / "validate_results.py", run_dir], cwd=package_dir, env=env)
        ...
        execution_manifest = {
            ...
            "wrapper_log": str(preliminary_log.relative_to(repo)),
            "unit_test_log": str(unit_log.relative_to(repo)),
            "completed_utc": utc_stamp(),
        }
        (run_dir / "WRAPPER_EXECUTION_MANIFEST.json").write_text(json.dumps(execution_manifest, indent=2) + "\n")
        shutil.copy2(unit_log, run_dir / "UNIT_TEST_LOG.txt")

        wrapper_path = Path(__file__).resolve()
        if repo not in wrapper_path.parents:
            raise RuntimeError(f"The wrapper must be dropped inside {repo}; current wrapper path is {wrapper_path}")
        stage_review_artifacts(runner, repo, package_dir, run_dir, wrapper_path)
        commit = commit_and_push(runner, repo, run_dir)

        verdict = json.loads((run_dir / "FINAL_GATE_VERDICT.json").read_text())
        runner.note("=" * 78)
        runner.note(f"RUN COMPLETE: {run_dir}")
        runner.note(f"AUTOMATED VERDICT: {verdict.get('verdict')}")
        runner.note(f"GIT COMMIT: {commit}")
        runner.note("The VS Code terminal remains open. Return the commit hash or run directory for scientific review.")
        return 0
