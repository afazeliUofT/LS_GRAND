#!/usr/bin/env python3
"""Single WSL wrapper for LS-GRAND G3P Structured/Matched/Physical Precursor v1.0.
...
PACKAGE_FILENAME = "LS_GRAND_G3P_Structured_Matched_Physical_Precursor_v1_0.zip"
PACKAGE_DIRNAME = "LS_GRAND_G3P_Structured_Matched_Physical_Precursor_v1_0"
EXPECTED_SHA256 = "9a645338d05b985e4ebc4c027f00a936b4107f914d02d2ad851333a36b5c06af"
EXPECTED_REPOSITORY = "afazeliUofT/LS_GRAND"
EXPECTED_REPO_PATH = Path("/home/afazeli2006/LS_GRAND")
REQUIRED_BASE_COMMIT = "0d2866b091576fe07521172378ded33da79ed545"
...
def candidate_download_dirs() -> list[Path]:
    out: list[Path] = []
    override = os.environ.get("LSGRAND_G3P_DOWNLOADS")
    if override:
...
def locate_package(runner: Runner) -> Path:
    ...
    if not found:
        ...
        raise FileNotFoundError(
            ...
            "Download the ZIP without renaming it, or set LSGRAND_G3P_DOWNLOADS."
        )
    valid: list[Path] = []
...
def ensure_root_gitignore(repo: Path) -> None:
    path = repo / ".gitignore"
    marker = "# LS-GRAND G3P precursor local execution"
    block = f"""
{marker}
.venv/
__pycache__/
...
def validate_profile() -> str:
    profile = os.environ.get("LSGRAND_G3P_PROFILE", "gate").strip().lower()
    if profile not in VALID_PROFILES:
        raise ValueError(f"LSGRAND_G3P_PROFILE must be one of {sorted(VALID_PROFILES)}, got {profile!r}")
    return profile
...
def commit_and_push(runner: Runner, repo: Path, run_dir: Path) -> str | None:
    ...
    runner.run(["git", "commit", "-m", f"LS-GRAND G3P structured matched physical precursor: {run_dir.name}"], cwd=repo)
    commit = runner.capture(["git", "rev-parse", "HEAD"], cwd=repo)
    if os.environ.get("LSGRAND_G3P_SKIP_PUSH", "0") == "1":
...
def main() -> int:
    stamp = utc_stamp()
    log_path = EXPECTED_REPO_PATH / "results" / "_wrapper_logs" / f"g3p_precursor_wrapper_{stamp}.log"
    runner = Runner(log_path)
    failure_path = log_path.with_name(f"G3P_PRECURSOR_WRAPPER_FAILURE_{stamp}.json")
    try:
        runner.note("=" * 78)
        runner.note("LS-GRAND G3P Structured/Matched/Physical Precursor v1.0")
        runner.note("=" * 78)
        ...
        unit_log = repo / "results" / "_wrapper_logs" / f"g3p_precursor_unit_tests_{stamp}.log"
        runner.run([
            python, "-m", "lsgrand_focused.cli", "test",
            "--package-root", package_dir,
            "--log", unit_log,
        ], cwd=package_dir, env=env)

        run_name = f"LS_GRAND_G3P_Structured_Matched_Physical_Precursor_{profile}_{stamp}"
        output_root = repo / "results"
        config = package_dir / "configs" / f"{profile}.json"
        runner.run([
            python, "-m", "lsgrand_focused.cli", "run",
            "--package-root", package_dir,
            "--config", config,
        ...
        verdict = json.loads((run_dir / "FINAL_G3P_VERDICT.json").read_text())
        runner.note("=" * 78)
        runner.note(f"RUN COMPLETE: {run_dir}")
        runner.note(f"AUTOMATED PRECURSOR CLASSIFICATION: {verdict.get('verdict')}")
