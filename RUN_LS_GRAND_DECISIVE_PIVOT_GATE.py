#!/usr/bin/env python3
"""Single WSL wrapper for LS-GRAND Decisive Pivot Gate v1.1.

Drop this file into /home/afazeli2006/LS_GRAND and run it from the existing
VS Code WSL terminal.  The script never executes a terminal-close command.
"""
...
PACKAGE_FILENAME = "LS_GRAND_Decisive_Pivot_Gate_v1_1.zip"
PACKAGE_DIRNAME = "LS_GRAND_Decisive_Pivot_Gate_v1_1"
EXPECTED_SHA256 = "aff5632155b7df18883e25553eb9d60823d68ccc56b22f4c0df371fb6fddbf7c"
EXPECTED_REPOSITORY = "afazeliUofT/LS_GRAND"
EXPECTED_REPO_PATH = Path("/home/afazeli2006/LS_GRAND")
REQUIRED_BASE_COMMIT = "531fdae2c27cb798edfe1bff6a8269f2fa341e29"
...
def candidate_download_dirs() -> list[Path]:
    out: list[Path] = []
    override = os.environ.get("LSGRAND_V11_DOWNLOADS")
    if override:
...
def locate_package(runner: Runner) -> Path:
    ...
    if not found:
        ...
        raise FileNotFoundError(
            ...
            "Download the ZIP without renaming it, or set LSGRAND_V11_DOWNLOADS."
        )
    valid: list[Path] = []
    ...
    if not valid:
        raise RuntimeError(
            "A ZIP with the expected name was found, but its SHA-256 is not the frozen package hash. "
...
def ensure_root_gitignore(repo: Path) -> None:
    path = repo / ".gitignore"
    marker = "# LS-GRAND decisive pivot local execution"
    block = f"""
{marker}
.venv/
__pycache__/
...
def validate_profile() -> str:
    profile = os.environ.get("LSGRAND_V11_PROFILE", "gate").strip().lower()
    if profile not in VALID_PROFILES:
        raise ValueError(f"LSGRAND_V11_PROFILE must be one of {sorted(VALID_PROFILES)}, got {profile!r}")
    return profile
...
def commit_and_push(runner: Runner, repo: Path, run_dir: Path) -> str | None:
    ...
    runner.run(["git", "commit", "-m", f"LS-GRAND decisive pivot gate: {run_dir.name}"], cwd=repo)
    commit = runner.capture(["git", "rev-parse", "HEAD"], cwd=repo)
    if os.environ.get("LSGRAND_V11_SKIP_PUSH", "0") == "1":
...
def main() -> int:
    stamp = utc_stamp()
    log_path = EXPECTED_REPO_PATH / "results" / "_wrapper_logs" / f"decisive_pivot_wrapper_{stamp}.log"
    runner = Runner(log_path)
    failure_path = log_path.with_name(f"DECISIVE_PIVOT_WRAPPER_FAILURE_{stamp}.json")
    try:
        runner.note("=" * 78)
        runner.note("LS-GRAND Decisive Pivot Gate v1.1")
        runner.note("=" * 78)
        ...
        unit_log = repo / "results" / "_wrapper_logs" / f"decisive_pivot_unit_tests_{stamp}.log"
        runner.run([python, "-m", "lsgrand_decisive.cli", "test", "--log", unit_log], cwd=package_dir, env=env)

        run_name = f"LS_GRAND_Decisive_Pivot_Gate_{profile}_{stamp}"
        output_root = repo / "results"
        config = package_dir / "configs" / f"{profile}.json"
        runner.run([
            python, "-m", "lsgrand_decisive.cli", "run",
            "--config", config,
        ...
        verdict = json.loads((run_dir / "FINAL_DECISIVE_VERDICT.json").read_text())
        runner.note("=" * 78)
        runner.note(f"RUN COMPLETE: {run_dir}")
        runner.note(f"AUTOMATED CLASSIFICATION: {verdict.get('verdict')}")
